...
trait NumbersStrategy {
    fn ui(&self, ui: &mut egui::Ui, app: &mut App);
...
impl NumbersStrategy for RealNumbersStrategy {
    ...
    fn ui(&self, ui: &mut egui::Ui, app: &mut App) {
        ui.horizontal_wrapped(|ui| {
            // ui.add(egui::DragValue::new(&mut self.number));
            ui.add(egui::DragValue::new(&mut app.numerator));
            ui.add(egui::DragValue::new(&mut app.denominator));
            ...
            if ui.button("Add").clicked() {
                app.fractions
                    .push((app.numerator, app.denominator.try_into().unwrap()));
            }
        });
    }
}
